...
<div class="max-w-4xl mx-auto px-6 py-10">
    <div class="ios-card p-6 md:p-8">
        ...
        <div class="ios-card p-5 mb-5">
            <form method="GET" action="{{ route('izin.status') }}" class="grid grid-cols-1 md:grid-cols-6 gap-3">
                <div class="md:col-span-2">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari jenis / alasan izin"
                ...
                <div>
                    <select name="status"
                        class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-100">
                ...
                <div>
                    <select name="sort"
                        class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-100">
                        <option value="latest" {{ request('sort', 'latest' )==='latest' ? 'selected' : '' }}>Tanggal
                            Terbaru</option>
                        <option value="oldest" {{ request('sort')==='oldest' ? 'selected' : '' }}>Tanggal Terlama
                        </option>
                    </select>
                </div>
                <div class="md:col-span-6 flex gap-2">
                    <button type="submit"
                        class="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Terapkan</button>
                    <a href="{{ route('izin.status') }}"
                        class="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Reset</a>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-xs text-gray-600 max-w-[280px] truncate">{{ $izin->alasan_izin ??
                                $izin->keterangan }}</td>
                            ...
                            <td class="px-4 py-3">
                                @if($izin->bukti_foto)
                                <a class="text-blue-600 hover:underline font-medium" target="_blank"
                                    href="{{ '/storage/' . $izin->bukti_foto }}">Lihat</a>
                                @else
                            ...
                            <td class="px-4 py-3">
                                ...
                                <a href="{{ route('izin.print', $izin->id) }}" target="_blank"
                                    class="inline-block rounded-lg bg-black px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90">
                                    Cetak
                                </a>
                                @else
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500">Data pengajuan tidak ditemukan.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <a href="/pengajuan"
            class="inline-block rounded-xl bg-blue-600 px-6 py-2 font-semibold text-white hover:bg-blue-700">
            Ajukan Izin Baru
        </a>
    </div>
</div>
